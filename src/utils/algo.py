...
# It writes label inplace
# argsort ?
def sorted_perm(a, label, reverse=False):
    # np.asarray on thus tuple lead to error, if label is string
    # because a shoulf be used as elementwise comparison
...
class Algo(Object):
    ...
    def clusters(self, state=dict(), true_order=True):
    ...
    def partition(self, state=dict()):
        ...
        K = self.K
        partition = dict(zip(*[np.arange(K), clusters]))
        return partition
...
class Annealing(Algo, frontendNetwork):
    ...
    def energy(self, state=dict(), get_params=False):
        ...
        L_o = 2 * (K * np.asarray(map(len, pi)).astype(float) - L_i)

        if get_params is True:
            return I, L_i, np.asarray(O), L_o

        # Total Energy
        E = 1/float(C) * ((I/L_i).sum() - (O/L_o).sum())
        #return 1/(1+np.exp(-E))
    ...
    def concentrate_clases(self, state=dict()):
        ...
        clusters = self.clusters(state, true_order=False, )
        ...
        for k in np.random.choice(range(K), K, replace=False):
            ...
            for c in np.arange(C):
                ...
                ## Delta Energy
                #pt = [c, old_c]
                #e_k_in_c[k, c] = 1/float(C) * ((_I/_L_i).sum() - (_O/_L_o).sum() - ((I[pt]/L_i[pt]).sum() - (O[pt]/L_o[pt]).sum()))
